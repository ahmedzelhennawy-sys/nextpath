// Ported from nextpath-main/src/ai-search.ts — this is the *only* AI-flavored logic
// in the UI. It is rendered with the violet "AI assist" visual treatment so the
// user can always tell it apart from the rule-based engine results.
...
export function parseNaturalLanguageQuery(query: string): ParsedSearchQuery {
  ...
  return {
    ...
    explanation: `Interpreted as: ${
      filters.fundingType || "all funding"
    } ${filters.type || "opportunities"} in ${
      filters.locationCountry || "any location"
    } for field '${filters.field || "all fields"}'.`,
  };
}
